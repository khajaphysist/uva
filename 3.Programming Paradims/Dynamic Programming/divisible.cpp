...
int run(int i, int m, int rem){
  ...
  if ((rem+a[i]) < 0) {
    if ((rem+a[i])%d == 0) {
      k = 0;
    }else k = d * (abs((rem+a[i])/d)+1) + rem+a[i];
  }else k = (rem+a[i])%d;

  int x = run(i+1,m-1,k) + run(i+1,m,rem) + (k == 0 && m==1 ? 1:0);
  ltb[i][m][rem] = x;
  return x;
}
...
int main(){
  ...
  while (cin >> n >> q,n) {
    ...
    count++;
    int ccount = 1;
    memset(ltb,-1,sizeof(ltb));
    while (q--) {
      cin >> d >> m;
      cout << "QUERY " << ccount << ": " << run(0,m,0) << endl;
      ccount++;
      memset(ltb,-1,sizeof(ltb));
    }
  }
