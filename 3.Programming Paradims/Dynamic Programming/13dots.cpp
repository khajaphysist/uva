...
int m,n;
int temp,temp1;
int run(int i, int mo){
  if (temp!=0 && i == n && temp1 <= mo && mo < 200 )return -999999;
  if (i == n || mo == 0) return 0;

  if (tab[i][mo]!=-1)return tab[i][mo];
  ...
  int ans = max(run(i+1,mo),run(i+1,mo-p[i])+f[i]);
  tab[i][mo] = ans;
  return ans;
}
...
int main(){
  ...
  while (scanf("%d %d",&m,&n)!=EOF) {
    ...
    if(m<=1800)temp = 0;
    if(m>2000) m+=200;
    if(m>1800 && m <=2000){temp = m;m+=200;temp1=m-2000;}
